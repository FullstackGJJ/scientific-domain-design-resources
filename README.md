...
**edit: I've updated `Rules` concept into `Concerns` concept so that it can be more flexible and scalable and labelled better. (Think <XYZ-concerns> instead of <XYZ-rules-of-thumb>) (I am now experiementing with concept of `Reasoning` or `Logic` i.e. <XYZ-reasoning> or <XYZ-logic> instead of concerns or rules of thumb)
